...
Write-Host ("  " + $GREEN + "[OK]" + $NC + " Gateway source installed")
...
if ($LOCAL_MODEL -and $OLLAMA_AVAILABLE) {
    ...
    if ($modelFound) {
        Write-Host ("  " + $GREEN + "[OK]" + $NC + " Using " + $BOLD + $LOCAL_MODEL + $NC + " for chat (no API key needed)")
    } else {
    ...
    if ($LOCAL_MODEL) {
        ...
        try {
            ...
            Write-Host ("  " + $GREEN + "[OK]" + $NC + " Model " + $LOCAL_MODEL + " ready")
            ...
            Write-Host ("  " + $RED + "[X]" + $NC + " Failed to pull " + $LOCAL_MODEL)
            $LOCAL_MODEL = ""
        }
    } else {
...
if ((Test-Path $CONFIG_FILE) -and $IS_UPGRADE) {
    Write-Host ("  " + $GREEN + "[OK]" + $NC + " Existing config preserved")
} else {
    & node (Join-Path $GATEWAY_DIR "src" "index.js") init 2>&1 | Out-Null
    Write-Host ("  " + $GREEN + "[OK]" + $NC + " Config created at " + $CONFIG_FILE)
}
...
# Create batch wrapper (avoid here-string with @echo for PS 5.1 compatibility)
$wrapperPath = Join-Path $BIN_DIR "claudia-gateway.cmd"
$wrapperContent = '@echo off' + "`r`n" + 'node "%USERPROFILE%\.claudia\gateway\src\index.js" %*'
    ...
    if ($platformChoice -eq "1") {
        ...
        if ($botToken) {
            ...
            while ($true) {
                ...
                Write-Host ""
                Write-Host ("  " + $RED + "[X]" + $NC + " That looks like a username, not a numeric ID.")
            ...
            if ($userId) {
                ...
                Write-Host ""
                Write-Host ("  " + $GREEN + "[OK]" + $NC + " Telegram configured in gateway.json")
                Write-Host ("  " + $GREEN + "[OK]" + $NC + " User " + $userId + " added to allowlist")
                Write-Host ""
                ...
                if ($profileContent -match 'TELEGRAM_BOT_TOKEN') {
                    ...
                    Write-Host ("  " + $GREEN + "[OK]" + $NC + " Bot token updated in PowerShell profile")
                } else {
                    Add-Content -Path $profilePath -Value "`n# Claudia Gateway - Telegram`n$tokenLine"
                    Write-Host ("  " + $GREEN + "[OK]" + $NC + " Bot token saved to PowerShell profile")
                }
        ...
        if ($slackBot -and $slackApp -and $slackUser) {
            ...
            Write-Host ""
            Write-Host ("  " + $GREEN + "[OK]" + $NC + " Slack configured in gateway.json")
            Write-Host ("  " + $GREEN + "[OK]" + $NC + " User " + $slackUser + " added to allowlist")
            ...
            if ($profileContent -match 'SLACK_BOT_TOKEN') {
                ...
                Set-Content -Path $profilePath -Value $profileContent
                Write-Host ("  " + $GREEN + "[OK]" + $NC + " Slack tokens updated in PowerShell profile")
            } else {
                Add-Content -Path $profilePath -Value "`n# Claudia Gateway - Slack`n$sbotLine`n$sappLine"
                Write-Host ("  " + $GREEN + "[OK]" + $NC + " Slack tokens saved to PowerShell profile")
            }
        } else {
            Write-Host ""
...
if (-not $LOCAL_MODEL) {
    ...
    Write-Host ("  " + $GREEN + "[OK]" + $NC + " Local model " + $LOCAL_MODEL + " (no API key needed)")
}

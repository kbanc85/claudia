...
- **Native Codex installation, in-place upgrades, and Voice-ready runtime.** Open a destination folder in Codex and run plain `npx get-claudia@latest`: the installer detects the active Codex thread and targets the open folder. It creates a complete new Claudia workspace when none exists, or refreshes an existing workspace and its plugin in place while preserving user context and memory. Both paths configure the memory daemon, repository marketplace, official Codex plugin, 34 Codex-adapted skills, and lifecycle hooks automatically. Explicit `npx get-claudia codex [dir]` remains available, and bare installs outside Codex retain the established `./claudia` default. `AGENTS.md` provides the required Codex startup contract; the optional SessionStart hook injects the compact memory briefing, while the optional SessionEnd hook queues Codex rollout transcripts for the existing sole-writer daemon. The parser understands Codex `response_item` messages and records them as `source_channel=codex`. The `claudia` shell helper remembers the selected host and adds explicit `codex`, `claude`, and `voice` surfaces.

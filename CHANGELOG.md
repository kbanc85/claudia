...
- **Native Codex installation and Voice-ready runtime.** Open a destination folder in Codex and run plain `npx get-claudia@latest`: the installer detects the active Codex thread, targets the open folder, and completes Claudia's workspace, memory daemon, repository marketplace, official Codex plugin, 34 Codex-adapted skills, and lifecycle-hook setup automatically. Explicit `npx get-claudia codex [dir]` remains available, and bare installs outside Codex retain the established `./claudia` default. `AGENTS.md` provides the required Codex startup contract; the optional SessionStart hook injects the compact memory briefing, while the optional SessionEnd hook queues Codex rollout transcripts for the existing sole-writer daemon. The parser understands Codex `response_item` messages and records them as `source_channel=codex`. The `claudia` shell helper remembers the selected host and adds explicit `codex`, `claude`, and `voice` surfaces.

...
- **Genericized example names in tests, docs, and examples.** Fixture people and companies now use fictional names throughout. Behavior is unchanged.

## 1.68.0 (2026-08-13)

### Added

- **One smart installer for Codex, Claude Code, and Grok.** Open a destination folder in any supported agent and run plain `npx get-claudia`: the installer checks npm's `latest` release, detects the active runtime from session-scoped signals, and targets the open folder. It creates a complete new Claudia workspace when none exists, or refreshes an existing workspace and host integration in place while preserving user context and memory. Explicit `npx get-claudia codex|claude|grok [dir]` overrides remain available, and bare installs outside an agent retain the established `./claudia` default. The shell helper remembers the selected runtime and can launch all three hosts. Grok uses its documented Claude Code compatibility layer to load Claudia's instructions, skills, hooks, agents, and `.mcp.json`.
- **Required Codex onboarding and Claudia identity.** `AGENTS.md`, `claudia-core`, the new Codex `onboarding` skill, and SessionStart context now require a real first-conversation setup whenever `context/me.md` is absent. Substantive work waits until the user-approved profile is written; upgrades with an existing profile skip the gate. Within the Claudia workspace, the agent identifies itself as Claudia rather than ChatGPT, Codex, or a generic assistant.
- **Native Codex plugin and Voice-ready runtime.** Codex installs Claudia's repository marketplace, official plugin, Codex-adapted skill catalog, and optional lifecycle hooks automatically. The optional SessionStart hook injects the compact memory briefing, while the optional SessionEnd hook queues Codex rollout transcripts for the existing sole-writer daemon. The parser understands Codex `response_item` messages and records them as `source_channel=codex`; ChatGPT Voice can coordinate a task in the configured Codex workspace.
...
- **Approved judgment rules were never applied.** `/meditate` writes rules to `context/judgment.yaml` and nothing read them. The `judgment-awareness` skill said it *"Activates at session start."* It does not: skills load only when invoked, nothing invoked this one, and no user has a reason to type "judgment awareness". So the default experience was to approve a rule, watch it be written to a file, and have it never influence another session. The writing half worked, which is what let this survive. Found in an install that had reached 132 rules, **27 of which had never been loaded into a single session**, with two IDs silently reused because the only way to pick the next one was to read a summary that was itself incomplete. Rules now live in `.claude/rules/judgment-active.md`, which is read into every session, regenerated by a new `judgment-sync` SessionStart hook.
- **Rule IDs had no uniqueness guarantee.** `meditate` said to assign "a sequential ID within its category", but IDs do not respect section boundaries in practice: in a real file 29 rules sat under the wrong section. Counting within a category therefore reads the wrong set and collides with a rule filed elsewhere. IDs are now validated across the whole file, including the `meta` section, and `/meditate` takes the next ID from the script instead of counting by hand.
- **`npm test` has not run since Node began resolving a bare `test/` argument as a module.** The script now targets `test/*.test.js`. The 37 tests behind it were passing the whole time; nothing was running them.
- **A real client's name shipped in the package.** Two always-on rule files used an actual client as their worked example, so every install carried that relationship in its context. Replaced with a placeholder, matching the fictional register the rest of the docs already use.
- **Compiled Python bytecode shipped in the tarball.** `__pycache__` directories were being packed, sending ~72KB of machine-specific `.pyc` files to every user. Now git-ignored and excluded.
- **Hook timeouts were declared in the wrong unit.** The settings schema defines `timeout` in seconds and every hook passed a millisecond value, so a "10 second" hook was configured for nearly three hours and no hook could ever time out. Note this is a behaviour change on update: a hook that genuinely runs long can now be stopped.

### Added

- **Tiered judgment loading, so the fix above does not become an unbounded context cost.** `.claude/rules/` is read into every session, so simply loading everything traded a silent failure for a growing one (132 rules is roughly 9,500 tokens, rising by about one rule per session). Rules are now carried by how expensive they are to miss: safety gates and meta-rules **verbatim**; most rules as a **one-line index**, the default; and rules explicitly tagged `domain:` as an **ID only**, grouped under that domain, so context still says "12 rules govern video" and how to fetch them. Misclassification costs detail, never awareness, and safety rules are pinned by section *and* ID prefix so neither a stray tag nor a filing slip can quietly demote an escalation gate. Deliberately not built on skill triggers, since that is the mechanism that failed.
- **`judgment-sync.py show <id>`** prints any rule in full with its provenance, which is what makes the shortened tiers safe to read.
- **A token budget, measured every session**, that names which rules to demote when the file grows past it. Without a number this file silently regrows.
- **Drift detection.** The 27 missing rules produced no warning and no symptom; they surfaced only during an unrelated audit. Drift is now reported at session start, measured against the generated section alone so a rule mentioned in passing in hand-written prose cannot mask one that is genuinely absent.

### Changed

- **`judgment-awareness`** no longer claims to load rules, and its documented schema now matches what `meditate` actually writes. The two had diverged: the doc showed `when`/`action`/`condition`, the skill writes `rule`/`context`. Both are still read, so an install that followed either keeps working.
- **The judgment parser needs no third-party package.** Every other hook in the template is stdlib-only, and a SessionStart hook that died on a missing `pip` package would take the judgment system down on exactly the machines least able to diagnose it. PyYAML is used when present; the stdlib fallback was verified to produce byte-identical output across a real 132-rule file, including folded block scalars. A file the fallback rescues but PyYAML rejects is now **reported rather than silently accepted**, because resilience that hides corruption is worse than a clean failure.

### Stats

- 48 new tests, weighted toward the invariant that no rule can vanish rather than toward formatting. Hooks suite green; 37 node tests green.
- No schema migration. Existing installs keep any hand-written content above the generated heading, and a generated section under an older heading is replaced rather than appended to.
...
Five PRs that fix the memory layer's biggest recurring failure mode and lock in the integration philosophy. After this release, memory writes that name entities ("Alex Rivera") actually create those entities with the correct type. The release history's recurring memory-fix releases ("Recall Recovery", "Vector Search Fix", "Semantic Search Actually Works Now") get permanent regression-test sentinels so the same bug classes can't quietly come back. And the codebase loses a dual-maintenance hazard that was already costing time.

#### Fixed
- **`memory_remember` actually links entities and infers their type correctly (#54)** -- A confirmed bug from 2026-05-13: calling `memory_remember(content="Alex Rivera said X", entities=["Alex Rivera", "Nimbus AI"])` was creating entities but assigning them `type: person` by default, even when the name clearly indicated an organization. "Nimbus AI" was being saved as a person. The real bug was in `_infer_entity_type` -- it didn't recognise `AI` / `.ai` / `Co.` as corporate suffixes and fell back to `person`. Fixed with a pure-function rule-based type inference (corporate suffixes -> organization, project keywords -> project, person patterns -> person, fallback -> concept, never default to person). Plus a new `claudia-memory --backfill-entities` CLI to retroactively link orphaned references in existing user databases.
...
- **Fuzzy entity dedup on write** -- `_ensure_entity()` and `_find_or_create_entity()` now perform a fuzzy pre-check (SequenceMatcher > 0.90) before creating new entities. Name variants like "Chris Brisko" vs "Chris Brisco" (ratio ~0.92) match the existing entity instead of creating a duplicate. Only compares entities of the same type, skips deleted entities.
...
#### Added
- **Edge bundling** - Tightly interconnected clusters (e.g., YPO Forum Alpha) now render bundled "highway" edges instead of spaghetti. Uses force-directed edge bundling (FDEB) as a post-processing pass after simulation settles. Configurable strength, radius, and endpoint stiffness via design panel.

...
try {
    ...
    if ($nodeVersion -match "v(\d+)\.") {
        ...
        if ($nodeMajor -ge 18) {
            ...
            Write-Host "  ${RED}✗${NC} Node.js $nodeVersion (v18+ required)"
...
try {
    ...
    Write-Host "  ${GREEN}✓${NC} npm v$npmVersion"
} catch {
    Write-Host "  ${RED}✗${NC} npm not found"
...
if (Test-Path $serverJs) {
    ...
    Write-Host "  ${GREEN}✓${NC} API backend installed"
} else {
    Write-Host "  ${RED}✗${NC} Source visualizer not found at $SOURCE_DIR"
    exit 1
}
...
if (Test-Path $threeJsPackage) {
    ...
    Write-Host "  ${GREEN}✓${NC} 3D frontend installed"
} else {
    Write-Host "  ${RED}✗${NC} Source visualizer-threejs not found at $THREEJS_SOURCE"
    exit 1
}
...
$ErrorActionPreference = "Continue"
Pop-Location
Write-Host "  ${GREEN}✓${NC} Backend dependencies ready"
...
$ErrorActionPreference = "Continue"
Pop-Location
Write-Host "  ${GREEN}✓${NC} Frontend dependencies ready"
...
try {
    ...
    $apiRunning = $true
    Write-Host "${GREEN}✓${NC} API server already running on port 3849"
} catch {
    ...
    try {
        $null = Invoke-WebRequest -Uri "http://localhost:3849/health" -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop
        Write-Host "${GREEN}✓${NC} API server started on port 3849"
    } catch {
...
try {
    ...
    $frontendPort = "5173"
    Write-Host "${GREEN}✓${NC} Frontend already running on port 5173"
} catch {
    try {
        $null = Invoke-WebRequest -Uri "http://localhost:5174" -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop
        $frontendPort = "5174"
        Write-Host "${GREEN}✓${NC} Frontend already running on port 5174"
    } catch {
        ...
        try {
            $null = Invoke-WebRequest -Uri "http://localhost:5173" -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop
            $frontendPort = "5173"
            Write-Host "${GREEN}✓${NC} Frontend started on port 5173"
        } catch {
            try {
                $null = Invoke-WebRequest -Uri "http://localhost:5174" -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop
                $frontendPort = "5174"
                Write-Host "${GREEN}✓${NC} Frontend started on port 5174"
            } catch {
...
Write-Host "  ${GREEN}✓${NC} Created launcher at $launcherPath"

# Also create a .cmd wrapper for easier CLI access
$cmdPath = Join-Path $BIN_DIR "brain.cmd"
$cmdContent = @"
@echo off
powershell.exe -ExecutionPolicy Bypass -File "%~dp0brain.ps1" %*
"@
Set-Content -Path $cmdPath -Value $cmdContent
Write-Host "  ${GREEN}✓${NC} Created CLI wrapper at $cmdPath"

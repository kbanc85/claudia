...
import { focusNode, filterNodes, resetFilter, getGraph } from './graph.js';
...
export function initUI(data) {
  ...
  // Expose showDetail for graph.js click handler
  window.__brainUI = { showDetail };
}
...
export function addSearchResults() {
  // Placeholder for external search result injection
}
...
function applyFilters() {
  filterNodes(node => {
    ...
    if (node.nodeType === 'pattern') return true; // always show patterns
    return true;
  });
}
...
export function showDetail(node, apiDetail) {
  ...
  // API detail (entity detail with memories, relationships, documents)
  if (apiDetail) {
    // Relationships
    if (apiDetail.relationships?.length > 0) {
      ...
      for (const rel of apiDetail.relationships) {
        ...
        // Click to focus on related entity
        item.addEventListener('click', () => {
          const targetNode = graphData?.nodes.find(n => n.id === `entity-${rel.other_id}`);
          if (targetNode) focusNode(targetNode, 800);
        });
    ...
    if (apiDetail.memories?.length > 0) {
      ...
      for (const mem of apiDetail.memories.slice(0, 15)) {
        ...
        item.addEventListener('click', () => {
          ...
          if (memNode) focusNode(memNode, 800);
        });
...
function initSettings() {
  ...
  for (const radio of radios) {
    radio.addEventListener('change', () => {
      if (radio.checked) {
        setQuality(radio.value);
      }
    });
  }

  // Sync radio to current quality (may have been loaded from localStorage)
...
function initTimeline() {
  ...
  slider.addEventListener('input', () => {
    ...
    // Filter graph to show only nodes created before cutoff
    if (pct >= 0.99) {
      resetFilter();
    } else {
      filterNodes(node => {

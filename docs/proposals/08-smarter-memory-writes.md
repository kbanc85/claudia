...
During the AIAC sponsorship work, three `memory.remember` calls saved facts about Alex Rivera (CEO Nimbus AI), Jordan (Head of Marketing Nimbus AI), and Nimbus AI itself. Each call included an `entities` array naming the relevant entities. After all three saves:

```
memory.about(entity="Alex Rivera")    → {entity: null, memories: [], relationships: []}
memory.entities(query="Alex Rivera")  → []
memory.entities(query="Jordan")          → []
memory.entities(query="Nimbus AI")      → []
```

The entities did not exist. The three facts were stored as floating memories with string labels that referenced nothing.

Only when `memory.relate` was called to link Alex Rivera to Nimbus AI did entity records get created — and Nimbus AI was created with `type: "person"` because that's the default for `memory.relate`'s auto-create path. The entity was misclassified the moment it came into being.
...
- LLM-based: small classifier (local Ollama call or rule-based heuristic) examines the entity name and the calling memory's content. "Nimbus AI" in a sentence about "AI content-quality tooling company" gets typed `organization`.
...
```python
content = "Alex Rivera said the enterprise track should be ..."
# NER extracts: ["Alex Rivera"]
# auto-links the memory to Alex Rivera entity (creating if missing)
```
...
- Integration: write a fact with `entities=["Alex Rivera", "Nimbus AI"]` → both entities are created with correct types → `memory.about("Alex Rivera")` returns the linked memory
...
- **Fuzzy match threshold.** "Alex" vs "Alex Rivera" vs "Alex R." — should these collapse? Recommend a separate `memory.entity.merge` tool for explicit merging rather than auto-collapse, which is risky.
